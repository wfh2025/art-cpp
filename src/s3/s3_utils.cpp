...
namespace
{
    ...
    public:
        ...
        std::tm& GetParsedTimestamp()
        {
            return m_parsedTimestamp;
        }
        bool ShouldIAssumeThisIsUTC() const
        {
            return m_utcAssumed;
        }
        const char* GetParsedTimezone() const
        {
            return m_tz;
        }
    ...
    protected:
        ...
        bool m_utcAssumed;
        // The size should be at least one byte greater than the maximum possible size so that we could use the last char to indicate the end of the
        // string.
        char m_tz[7];
    };
    ...
    // the standard is parsed correctly. strptime isn't available one Windows. This code gets hit pretty hard during http
    // serialization/deserialization as a result I'm going for no dynamic allocations and linear complexity
        ...
        void Parse() override
        {
            ...
            while (m_state <= finalState && !m_error && index < len)
            {
                ...
                case 5:
                    ...
                    break;
                case 6:
                    if ((c == 'Z' || c == '+' || c == '-') && (index - stateStartIndex >= 3) && (index - stateStartIndex <= 9))
                    {
                        m_tz[0] = c;
                        m_state = 7;
                        stateStartIndex = index + 1;
    ...
    private:
        ...
        static bool IsUTCTimeZoneDesignator(const char* str)
        {
            ...
            if (len > 0)
            {
                ...
                if (len == 6 && str[0] == '+' && str[1] == '0' && str[2] == '0' && str[3] == ':' && str[4] == '0' && str[5] == '0')
                {
                    return true;
                }
    ...
    private:
        ...
        static bool IsUTCTimeZoneDesignator(const char* str)
        {
            ...
            if (len > 0)
            {
                ...
                if (len == 5 && str[0] == '+' && str[1] == '0' && str[2] == '0' && str[3] == '0' && str[4] == '0')
                {
                    return true;
                }
...
namespace s3
{
    namespace utils
    {
        ...
        DateTime::DateTime(const std::string& timestamp, DateFormat format) : m_valid(true)
        {
        ...
        DateTime& DateTime::operator=(const std::string& timestamp)
        {
        ...
        std::string DateTime::ToLocalTimeString(DateFormat format) const
        {
            switch (format)
            {
        ...
        std::string DateTime::ToLocalTimeString(const char* formatStr) const
        {
        ...
        std::string DateTime::ToGmtString(DateFormat format) const
        {
            switch (format)
            {
            case DateFormat::ISO_8601:
            ...
            case DateFormat::RFC822: {
                ...
                std::string rfc822GmtString = ToGmtString(RFC822_DATE_FORMAT_STR_MINUS_Z);
                rfc822GmtString += " GMT";
        ...
        std::string DateTime::ToGmtString(const char* formatStr) const
        {
        ...
        std::string DateTime::ToGmtStringWithMs() const
        {
            struct tm gmtTimeStamp = ConvertTimestampToGmtStruct();

            char formattedString[100];
            std::strftime(formattedString, sizeof(formattedString), "%Y-%m-%dT%H:%M:%S", &gmtTimeStamp);
            std::string formattedStringStr = formattedString;
        ...
        {
            return static_cast<Month>(GetTimeStruct(localTime).tm_mon);
        }
        ...
        {
            return static_cast<DayOfWeek>(GetTimeStruct(localTime).tm_wday);
        }
        ...
        std::string DateTime::CalculateLocalTimestampAsString(const char* formatStr)
        {
            DateTime now = Now();
            return now.ToLocalTimeString(formatStr);
        }

        std::string DateTime::CalculateGmtTimestampAsString(const char* formatStr)
        {
            DateTime now = Now();
            return now.ToGmtString(formatStr);
        }

        std::string DateTime::CalculateGmtTimeWithMsPrecision()
        {
